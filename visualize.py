...
def generate_synthetic_data(num_vertices: int, spread_factor: float, output_file: str, random_seed: int = None) -> np.ndarray:
  ...
  # For two classes, we use -1 and 1 as in the original code.
  class_labels = [-1, 1] if NUM_CLASSES == 2 else list(range(NUM_CLASSES))
...
def run_command(cmd_args: List[str], cwd: str) -> None:
  """
...
def main():
  ...
  parser.add_argument("--spread_factor", type=float, help="Spread factor for the clusters", default=0.5)
  ...
  filter_exe = "./filter"
  gabriel_args = path_from_gabriel_to_root + synthetic_data_file_from_root
  filter_args = path_from_filter_to_root + gabriel_output_from_root
  gabriel_output = gabriel_output_from_root
  filter_output = filter_output_from_root
  
  # Generate synthetic data
  data = generate_synthetic_data(args.num_vertices, args.spread_factor, synthetic_data_file_from_root, args.seed)

  fig, axes = plt.subplots(1, 2, figsize=(12, 6))
  plot_synthetic_data(axes[0], data, title="Synthetic Data")
  
  # Run Gabriel Graph executable
  run_command([gabriel_exe, gabriel_args], gabriel_cwd)
  
  if not pathlib.Path(gabriel_output).exists():
    print(f"Error: Expected Gabriel Graph output not found at {gabriel_output}")
  else:
    graph = parse_graph_file(gabriel_output)
    plot_graph(axes[1], graph, title="Gabriel Graph")
  
  # Run Low Degree Filter executable
  run_command([filter_exe, filter_args], filter_cwd)
  
  if not pathlib.Path(filter_output).exists():
    print(f"Error: Expected Low Degree Filter output not found at {filter_output}")
  else:
    filtered_graph = parse_graph_file(filter_output)
