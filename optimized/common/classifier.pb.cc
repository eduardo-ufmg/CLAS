...
const char descriptor_table_protodef_classifier_2eproto[] PROTOBUF_SECTION_VARIABLE(protodesc_cold) =
  ...
  "tries\030\001 \003(\0132\031.classifierpb.ExpertEntry\"9"
...
uint8_t* ExpertEntry::_InternalSerialize(
  ...
  float tmp_bias = this->_internal_bias();
  uint32_t raw_bias;
  memcpy(&raw_bias, &tmp_bias, sizeof(tmp_bias));
  if (raw_bias != 0) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteFloatToArray(4, this->_internal_bias(), target);
  }
